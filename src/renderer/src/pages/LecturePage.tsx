...
import { ArrowLeft, BookOpen, Lightbulb, Pencil, Target, Sparkles, ChevronDown, ChevronRight } from 'lucide-react'
...
function ExampleCard({ example, index }: { example: LectureExample; index: number }) {
  ...
  return (
    <div className="rounded-lg border bg-card">
      <button
        className="flex w-full items-center gap-3 px-4 py-3 text-left font-medium text-sm"
      ...
      {expanded && (
        <div className="border-t px-4 py-3 space-y-3 text-sm">
          <div>
            ...
            <p className="mt-1 whitespace-pre-wrap">{example.problem}</p>
          </div>
          <div>
            <span className="font-medium text-muted-foreground">解答</span>
            <p className="mt-1 whitespace-pre-wrap">{example.solution}</p>
          </div>
          {example.explanation && (
            <div>
              <span className="font-medium text-muted-foreground">思路</span>
              <p className="mt-1 whitespace-pre-wrap text-muted-foreground">{example.explanation}</p>
            </div>
          )}
        </div>
      )}
  ...
  return (
    <div className="rounded-lg border bg-card p-4">
      <div className="flex items-start gap-3">
        <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded bg-green-100 text-xs font-bold text-green-700">
          {index + 1}
        </span>
        <div className="flex-1">
          ...
          {exercise.hint && !showAnswer && (
            <p className="mt-2 text-xs text-muted-foreground">提示: {exercise.hint}</p>
          )}
          <button
          ...
          {showAnswer && (
            <div className="mt-2 rounded bg-muted/50 p-3 text-sm whitespace-pre-wrap">
              {exercise.answer}
            </div>
          )}
        </div>
...
export default function LecturePage() {
  ...
  async function loadLecture(): Promise<void> {
    if (!taskId) return

    // Try to get existing lecture first
    const existing = await window.learnerAI.lecture.get(taskId) as Lecture | null
    if (existing) {
      setLecture(existing)
      return
    }
    ...
    setError(null)
    try {
      const generated = await window.learnerAI.lecture.generate(taskId) as Lecture
  ...
  if (error) {
    return (
      <div className="flex h-full flex-col items-center justify-center p-8 space-y-4">
        ...
        <Button variant="outline" onClick={loadLecture}>重试</Button>
      </div>
    )
  }
        ...
        <div>
          ...
          <div className="mt-1 flex items-center gap-3 text-sm text-muted-foreground">
            ...
            {lecture.prerequisites.length > 0 && (
              <>
                <span>·</span>
                <span>前置: {lecture.prerequisites.join('、')}</span>
              </>
            )}
            <span>·</span>
            <span>{new Date(lecture.generatedAt).toLocaleString('zh-CN')}</span>
          </div>
        </div>
