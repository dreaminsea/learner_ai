...
3. 知识图谱采用增量式构建：你只需要为第一个 Stage 的 Day 1 任务生成 2-5 个根知识点(knowledgeNodeRefs)，作为知识网络的起点。
   - 其他任务不需要填写 knowledgeNodeRefs（留空数组 []），知识点会在用户完成每个任务后由系统自动生成。
   - 根知识点必须是该学科最基础的核心概念，如"极限的 epsilon-delta 定义"、"数列收敛"等。
   - 禁止使用泛化词作为标签（如单独的"检测"、"总结"、"例子"、"基础"）。
