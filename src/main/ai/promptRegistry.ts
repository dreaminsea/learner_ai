...
4. 知识点要求：
   - 必须是具体、有信息量的概念/定理/方法名，如"柯西收敛准则"而不是"数列基础"
   - 标签长度至少 4 个字，禁止缩写
   - 禁止使用包含以下词汇的标签：基础、检测、测试、考试、练习、总结、复习、例子、示例、应用、概论、概述、入门
   - 检测(assessment)和项目(project)任务的 knowledgeNodeRefs 必须为空数组 []
   - 同一知识点最多被 5 个学习(learn)/练习(practice)任务引用
5. 知识图谱结构要求：
