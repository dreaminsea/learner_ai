...
3. 每个任务关联具体的知识点(knowledgeNodeRefs)，方便后续知识图谱构建。每个任务关联 1-3 个知识点。
4. 知识图谱结构要求：
   - 必须有至少一个根节点（第一个 stage 的 day 1 任务应引入全新概念）
   - 同一个 stage 内知识点不要全连接，只连接有直接依赖关系的
   - 复习(review)任务应引用已在之前任务中出现过的知识点
5. 时间估算要合理，每个任务不要超过 2 小时。
6. 使用用户指定的语言（中文/英文）来写内容。
